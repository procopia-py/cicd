        ...
        stage('Build and Push to Docker Hub') {
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'f3f01044-75e5-4889-80cc-bba6af514791', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME')]) {
                        sh """
        ...
        stage('Deploy to EC2') {
            steps {
                script { 
			      ...
			      scp -o StrictHostKeyChecking=no -r app docs docker-compose.yml Dockerfile prometheus.yml yarn.lock package.json ${EC2_USER}@${EC2_IP}:/home/ubuntu/docker
                            # Connect to the EC2 instance and execute commands
                            ssh -o StrictHostKeyChecking=no ${EC2_USER}@${EC2_IP} << EOF
                            cd ./docker
                            pwd
